...
def calculate_mixed(gene,gene_name, chr_no, match_num_total, head_num, inputfrag_len, tempfile_path):  ############   inputfrag_len = 2*150 + 100
            ...
            if dim != ():
                ...
                if len(dim) == 1:  #############################numpy数组的形状是一维的
                    ...
                    if flag1 == 1:
                        ...
                        # matrix_spot_count_set = set(matrix_spot_count)
                        # matrix_spot_count_set_num = np.zeros(len(matrix_spot_count_set))##自己去找环形isoform bsj reads
                        # matrix_spot_count_set_dict = dict(zip(matrix_spot_count_set,matrix_spot_count_set_num))
                        matrix_mixed_spot_count_set = set(matrix_mixed_spot_count)
                        matrix_mixed_spot_count_set_num = np.zeros(len(matrix_mixed_spot_count_set))##自己去找环形isoform bsj reads
                        matrix_mixed_spot_count_set_dict = dict(zip(matrix_mixed_spot_count_set,matrix_mixed_spot_count_set_num))
                        post_count_ratio_bsj = np.zeros(len(matrix_mixed))
                        for j in range(0, len(matrix_mixed)):
                                matrix_mixed_spot_count_set_dict[matrix_mixed[j][0][0] + matrix_mixed[j][-1][-1]] += total_count[j]
                        for j in range(0, len(matrix_mixed)):
                                post_count_ratio_bsj[j] = total_count[j]/matrix_mixed_spot_count_set_dict[matrix_mixed[j][0][0] + matrix_mixed[j][-1][-1]]
                        p = 0
                        post_count_ratio = list(post_count_ratio)
                        post_count_ratio_bsj = list(post_count_ratio_bsj)
                        # print("post_count_ratio_before = " + str(post_count_ratio) + "matrix_mixed_before = " + str(matrix_mixed))
                        while (p < len(post_count_ratio)):
                            # if float(post_count_ratio[p]) < 0.000001:##由 0.04 改为 0.03
                            if (p < circAST_spot_num) and (matrix_spot_count.count(matrix_mixed[p][0][0] + matrix_mixed[p][-1][-1]) != 1) and (float(post_count_ratio_bsj[p]) < 0.05):##0.05需要调试
                                matrix_mixed_spot_count.remove(matrix_mixed[p][0][0] + matrix_mixed[p][-1][-1])
                                post_count_ratio.pop(p)
                                post_count_ratio_bsj.pop(p)
                                matrix_mixed.pop(p)
                                isoform_len_mixed.pop(p)
                                circAST_spot_num -= 1
                                recount = 1
                                p -= 1
                            if (p >= circAST_spot_num) and (float(post_count_ratio[p]) < 0.001):
                                matrix_mixed_spot_count.remove(matrix_mixed[p][0][0] + matrix_mixed[p][-1][-1])
                                post_count_ratio.pop(p)
                                post_count_ratio_bsj.pop(p)
                                matrix_mixed.pop(p)
    ...
    matrix_spot_count = []
    for i in range(0,len(matrix)):
        matrix_spot_count.append(matrix[i][0][0] + matrix[i][-1][-1])
    ...
    matrix_mixed_spot_count = []
    for i in range(0,len(matrix_mixed)):
        matrix_mixed_spot_count.append(matrix_mixed[i][0][0] + matrix_mixed[i][-1][-1])
    ...
    for t in range(0,circ_spot_num):
        os.remove(tempfile_path + gene_id + "_" + str(t) + ".sam")  
    return circ_reads
def proceccSamFile(item,tempfile_path,circ_chr,a,divided_sam_file_num,divided_sam_file_threshold,pair_count,circ_specific_reads):
    CIGAR_set_circAST1 = ["M","I","N","D"]
    CIGAR_set2 = ["M","N","D"]
    ##SAM文件后缀
    suffix = str(item)
    ...
    while i != pair_count_part:
        ...
        if array1[2][0:3] != "chr":
            ...
            chr_name = array1[2]
        if (array1[0] in circ_specific_reads):
            if (array1[2] in circ_chr) and((int(array1[1]) == 145 and int(array2[1]) == 97) or (int(array1[1]) == 81 and int(array2[1]) == 161)):
        ...
        if array1[2] == array2[2]:
            if (int(array1[1]) == 145 and int(array2[1]) == 97) or (int(array1[1]) == 81 and int(array2[1]) == 161):
                ...
                flag = set(CIGAR).issubset(set(CIGAR_set_circAST1))
                if flag == True:
                    sam_total_part += 1
                    t11 = int(array1[3])
            ...
            if (int(array1[1]) == 99 and int(array2[1]) == 147) or (int(array1[1]) == 163 and int(array2[1]) == 83):
                ...
                if flag == True:
                    ...
                    if t12 < t22 and array2[2] in circ_chr:
                        ...
                        out_samA.close()
            if (array1[0] not in circ_name_2):
                out_sam2.write(line1 + "\n")
                out_sam2.write(line2 + "\n")
    in_sam.close()
    out_sam2.close()
    return sam_total_part


if __name__ == "__main__":
    parse = optparse.OptionParser()
    parse.add_option('-G','--gtf',dest = 'gtf',action = 'store',metavar = 'gtf file name',help = 'please enter your annotation gtf file')
    parse.add_option('-I','--file',dest = 'file',action = 'store',metavar = 'input files',help = 'enter a SAM alignment of pair-end reads which is generated by Hisat2 and should be sorted by name')
            ...
            if array[1] in chr_name_gtf:
                if array[8] == "exon":
                    ...
                    if start in left[k] and end in right[k]:
                        ...
                        if gene_name_same != []:
                            gene_name_same.sort()
                            for reads in array[-1].split(","):
                                circ_specific_reads.append(reads)
                            out_list = open(tempfile_path + "circRNA_list.txt", "a")
                            out_list.write(
    ...
    for line in in_txt:
        ...
        if array[0][0:3] != "chr":
            array[0] = "chr" + array[0]
        if array[0] in chr_name_gtf:
            out_txt = open(tempfile_path + "ciri_" + array[0] + ".txt", "a")
            out_txt.write(line + "\n")
            out_txt.close()
            ciri_dict_name.append(array[0])
            # for j in range(int(array[4])):
                # circ_name_ciri.add(array[-1][j])
            if array[0] not in circ_chr:
                circ_chr.append(array[0])
    in_txt.close()
    ...
    partial_func_proceccSamFile = partial(proceccSamFile, tempfile_path=tempfile_path, circ_chr=circ_chr, a=a, divided_sam_file_num=divided_sam_file_num, divided_sam_file_threshold=divided_sam_file_threshold, pair_count=pair_count,circ_specific_reads=circ_specific_reads)
